...
Цей розділ забезпечує CRUD-операції для керування вузлами та їх моніторингу. Зверніть увагу, що операції зміни стану вимагають прав суперкористувача.

### 4.1. CRUD операції
- **GET `/nodes`**: Отримання списку всіх доступних вузлів.
- **POST `/nodes`**: Реєстрація нового вузла (лише для суперкористувача).
- **GET `/nodes/{node_id}`**: Отримання детальної інформації про вузол.
- **PUT `/nodes/{node_id}`**: Часткове оновлення даних вузла (лише для суперкористувача).
- **DELETE `/nodes/{node_id}`**: Видалення вузла (лише для суперкористувача).

### 4.2. Статус вузла (Health)
**URL:** `GET /nodes/{node_id}/status`
Повертає останній відомий стан вузла, який періодично оновлюється фоновим поллером (`node_health`).

### 4.3. Пробудження (Wake-on-USB)
**URL:** `POST /nodes/{node_id}/ws/wake`

Використовується для відправки "магічного" сигналу клавіатури (наприклад, Left Shift) для пробудження цільового ПК.
На відміну від постійного WebSocket-з'єднання, цей запит **проксіюється бекендом** (API перенаправляє HTTP-запит до `kvm_engine_py` на відповідний вузол), що дозволяє розбудити хост ще до встановлення WebSocket.
